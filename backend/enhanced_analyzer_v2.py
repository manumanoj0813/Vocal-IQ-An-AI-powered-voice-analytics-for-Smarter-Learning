...
class LanguageDetector:
    ...
    def detect_language_from_audio(self, audio_path: str) -> Dict:
        ...
        try:
            ...
            if transcription_result and transcription_result["confidence"] > 0.85:
                logger.info("Using transcription-based result (high confidence)")
                return transcription_result
            elif feature_result["confidence"] > 0.7:
                logger.info("Using feature-based result (high confidence)")
                return feature_result
            elif transcription_result:
                logger.info("Using transcription-based result (moderate confidence)")
                return transcription_result
            else:
                logger.info("Using feature-based result (default)")
                return feature_result
    ...
    def _detect_language_from_features(self, audio_path: str) -> Dict:
        ...
        try:
            ...
            # English detection (refined thresholds)
            if 1400 < avg_centroid < 3800 and 2300 < avg_rolloff < 6500:
                language_scores['en'] += 3
            if 0.03 < avg_zcr < 0.22:
                language_scores['en'] += 2
            if 1100 < avg_bandwidth < 2000:
                language_scores['en'] += 2
            if 12 < mfcc_std < 30:
...
class AdvancedVoiceCloningDetector:
    ...
    def detect_voice_cloning(self, audio_path: str) -> Dict:
        ...
        try:
            ...
            # Optimized threshold for better accuracy
            is_ai_generated = final_confidence > 0.42
            
            # Determine risk level
            if final_confidence > 0.68:
                risk_level = "high"
            elif final_confidence > 0.48:
                risk_level = "medium"
            else:
                risk_level = "low"
